...
public class MainActivity extends AppCompatActivity implements ClickListeners {
    ...
    public void addLoadout (View view) {
        ...
        if(loadouts.size() < 8) {
            loadouts.add(new Loadout(classes.get(temp), temp,
    ...
    private void loadBuilds() throws IOException {
        ...
        while(loadReader.ready()) {
            lineData = new ArrayList<>(5);
            lineLoc = 0;
            for(int i = 0; i < 5; i++) {
                lineData.add(new StringBuilder());
            }
            while(loadReader.ready() && lineLoc < 5) {
                ...
                if(lineLoc == 5) {
                    ...
                    loadouts.add(new Loadout(classes.get(temps[0]), temps[0],
